...
TEACH_SOCKET_PATH = "/tmp/robot_control_socket" # Ph?i gi?ng v?i teach.py
...
def send_control_data_ipc(left_dir, left_speed, right_dir, right_speed, servo1_angle, servo2_angle):
    if ipc_client_socket:
        try:
            ...
            # C th? server chua s?n sng, v?i DGRAM th sendto khng bo l?i ngay n?u server chua bind
            print(f"Error sending control data via IPC: {e}")
            pass # Ho?c th? k?t n?i l?i n?u dg SOCK_STREAM
